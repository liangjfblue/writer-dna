...
从 wiki/craft/techniques.md 的开头模式中选取合适的开头方式。具体的开头技法示例见 `references/scenes/`。

### 逐一展示法

当涉及多个产品/案例对比时，逐一展示而非一次性罗列。具体的逐一展示技法见 `references/scenes/`。

### 创意案例

产品评测或工具推荐时，用让人"卧槽"的创意案例包装成微型故事。具体案例技法见 `references/scenes/`。

### 结构模板

文章基本结构为：开头（感性切入）→ 背景/科普 → 核心内容（分板块展开，每个板块有观点+支撑+私人视角+扣主线句）→ 升华（连接更大的文化/哲学/历史参照物）→ 收尾。具体的结构模板和收尾方式见 `references/scenes/`。

### 字数和格式

从 wiki/persona/tone-voice.md 提取用户的格式偏好。默认建议：
- 公众号长文一般在 4000-8000 字
- 段落要短，很多时候一句话就是一段
- 重要观点前后留白，让它"呼吸"
- 需要插图的位置标注具体的配图建议，格式为"图片：具体描述"，说明配什么图、从哪里找素材。例如："图片：ArXiv 论文中的幂律缩放散点图"或"图片：1924 年霍桑工厂实验历史照片"。不要只写"图片"二字，那对作者没有参考价值
- 小标题和结构化排版根据文章类型灵活处理，不是所有文章都需要或都不需要小标题

### 写作场景

根据 wiki/craft/techniques.md 中识别的文章原型，从 `references/scenes/` 中加载对应的场景插件。场景插件包含该文章类型的具体写作技法、开头模式、结构模板和收尾方式。

如果没有匹配的场景插件，使用通用骨架直接写作。

## 第四步：五层自检体系

写完后必须跑完整个五层质检流程。这个自检体系的设计理念来自软件工程中的测试金字塔，从最基础的硬性规则到最主观的活人感判断，层层递进。每一层都有明确的通过标准和修复指引。

**通过判定规则：**
- L1（硬性规则）和 L5（纠错记录）必须全部通过，未通过则必须修复
- L2-L4 为风格建议，不阻断输出，但会在质检报告中标注偏离程度

### L1 硬性规则（指令式，零容忍）

这一层是硬性底线，任何一项不通过就必须修复，没有例外。
...
**L1-4 工具名检查**
指令：确认所有提到的AI工具/产品都使用了具体名称，将"AI工具""某个模型""相关技术"等空泛表述替换为具体产品名。
通过标准：零空泛表述。

**通过标准**：以上四项扫描零命中。未通过则必须修复，阻断输出。

### L2 风格一致性（建议式，不阻断）

这一层提供风格建议，帮助文章更贴近目标写作模式。未达标不阻断输出，但标注偏离程度。

**L2-1 开头**
- 建议：从一个具体的、当下的事件/场景切入（避免宏大叙事）
- 建议：第一句话让读者产生"然后呢？"的冲动
- 建议：不使用教科书式开头

**L2-2 节奏与结构**
- 建议：每3句内穿插至少1句短句（≤15字），避免连续3句句式相似
- 建议：全文至少3处一句话独立成段制造"断裂"效果
- 建议：偏离主线的段落后用"扣主线句"拉回
- 建议：用疑问句制造节奏的刹车和转向
- 建议：避免不必要的小标题

**L2-3 口语化**
- 建议：使用8-10个不同的口语化表达（从推荐词组中选取）
- 建议：加入论述中的故意打破（重复强调、中途打断、省略主语等）
- 建议：至少一处自嘲或承认不足
- 建议：用标点表达情绪（"。。。""？？？""= ="至少出现一种）

**L2-4 标点禁令二次确认（指令式 — L1防回归检查）**
- 指令：确认全文无冒号、破折号和双引号（AI修改过程中容易重新引入）
- 通过标准：零命中。

**通过标准**：建议达标。未达标项在质检报告中标注为 ⚠️偏离，不阻断输出。

### L3 内容质量（建议式，不阻断）

这一层检查内容本身的深度和说服力。未达标不阻断输出，但标注偏离程度。

**L3-1 观点支撑**
- 建议：每个核心观点配备具体的人/场景/细节/数据支撑
- 建议：避免只有论断没有例证的空泛观点

**L3-2 知识输出方式**
- 建议：知识点以"聊着聊着顺手掏出来"的方式呈现
- 建议：避免"下面我来介绍""首先需要了解"等教科书式科普
- 建议：引用自然融入论述，像"聊着聊着想起来的"而非"特意去查的"

**L3-3 文化升维**
- 建议：至少一处从具体事件连接到更大的文化/哲学/历史参照物
- 建议：连接方式自然，像"聊着聊着想到的"而非硬凑

**L3-4 对立面与同理心**
- 建议：讲核心观点时，先站到读者的处境里，再给出自己的视角
- 建议：先承认对方立场的合理性，再切入不同视角

**L3-5 文章类型专项**
根据文章原型做针对性检查：
- 调查实验型 → 建议：有"亲自下场"的叙事感，过程有层层递进的发现
- 产品体验型 → 建议：有真实使用场景和与其他产品的自然对比
- 现象解读型 → 建议：有"观察→好奇→研究→升维"的推进
- 工具分享型 → 建议：有个人故事铺垫，效果展示让人"卧槽"
- 方法论分享型 → 建议：每节落到可执行行动，坦诚说明学习成本和失败点，有谦逊铺垫

**L3-6 逐一展示**
- 建议：涉及多产品/案例比较时，用逐一展示（每个带吐槽点评）而非一次性罗列

**通过标准**：建议达标。L3-1 和 L3-2 强烈建议通过，L3-3 至 L3-6 根据文章类型选择适用项。未达标标注为 ⚠️偏离，不阻断输出。

### L4 活人感（建议式，不阻断）

这一层以读者视角通读全文，判断文章的"活人感"。未达标不阻断输出，但标注偏离程度。

**L4-1 温度感**
- 建议：情绪表达使用体感记忆（"我当时就愣住了""鼻子一酸"）而非知识性描述（"我感到非常震撼"）
- 建议：人物描写让读者"能感觉到体温"

**L4-2 独特性**
- 建议：文章有"只有[作者]才会写出来的角度"
- 建议：避免换一个AI博主也能写出差不多的内容

**L4-3 姿态**
- 建议：语气是"一个有见识的普通人在认真聊一件打动他的事"
- 建议：避免滑入"导师在教学生"或"品牌在做营销"的姿态

**L4-4 心流**
- 建议：从头到尾读，注意力不断掉，不需要回头理解逻辑
- 建议：如果某个地方注意力断掉，那就是需要修复的节奏问题

**通过标准**：整体感知达标。如果任何一项觉得"这段AI味太重了"，标注为 ⚠️偏离，不阻断输出。

### L5 运行时纠错（动态最高优先级）

这一层记录用户在写作过程中给出的直接纠错反馈。当以下场景出现时，
L5 规则覆盖 L1-L4 的任何冲突规则。

{纠错记录由 writer-dna 从 wiki/craft/self-check.md 动态注入}

**通过标准**：无纠错记录时自动通过；有纠错记录时，匹配场景必须遵循纠错指令，未遵循则必须修复，阻断输出。
...
- 开头：✅达标 / ⚠️略偏离（具体：...）
- 节奏：✅达标 / ⚠️略偏离（具体：...）
- 口语化：✅达标（使用了X个口语词组）/ ⚠️偏低（仅X个，建议8-10个）
- 标点禁令二次确认：✅/❌
...
**L5 纠错记录** ✅/❌/无记录
- 纠错记录：X条，命中Y条场景（已遵循/待修复）

**总评**：L1+L5 全部通过，可输出 / L1或L5未通过，需返工
**风格偏离提示**：[如L2-L4有偏离，列出最需要关注的1-3项]
```
