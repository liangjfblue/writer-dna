...
WriterDNA 是一个写作风格蒸馏工具。你把原始素材（文章、聊天记录、喜欢的作者）扔进去，它分析你的写作风格，然后把风格"注入"一个经过验证的 Skill 骨架模板，最终生成一个可以安装到 Claude Code 的个人写作 Skill。
...
安装后，对 Claude 说"帮我写一篇关于 XX 的文章"，就会自动用你的风格写。
...
- **双层架构** — 骨架模板（写作框架）和风格数据（你的风格基因）分离，两层独立迭代，最终融合。换骨架不需要重新分析风格，更新风格不需要改骨架。
- **插件化场景** — 每种文章原型（科普、观点、实操...）是独立的场景插件，包含专属的开头模式、结构模板和收尾方式。新增类型零侵入，核心 SKILL.md 不动。
- **五层渐进式自检** — L1 硬性规则（零容忍）→ L2 风格一致性 → L3 内容质量 → L4 活人感 → L5 运行时纠错。L5 是杀手级功能：写作中说"这句不像我"，自动记录，优先级高于 L1。
...
- [数字生命卡兹克](https://github.com/KKKKhazix/Khazix-Skills) — 骨架模板和五层自检体系
